...
// Colab / Local Drive File Explorer Endpoint (No OAuth Required)
app.get("/api/colab-drive/list", async (req: any, res: any) => {
  try {
    const requestedPath = (req.query.dirPath as string | undefined)?.trim();
    
    // Determine candidate root directories for Colab / Local environment
    ...
    const isDriveMounted = fs.existsSync("/content/drive/MyDrive") || fs.existsSync("/content/drive");
    ...
    if (!fs.existsSync(targetDir)) {
      return res.json({
        ...
        error: `مسیر مورد نظر یافت نشد: ${targetDir}`,
        folders: [],
        files: []
      });
    }
    ...
    if (!stat.isDirectory()) {
      return res.json({
        ...
        isDriveMounted,
        currentPath: targetDir,
        error: `مسیر انتخابی یک پوشه نیست.`,
        folders: [],
        files: []
      });
    }
    ...
    const files: Array<{ name: string; path: string; size: number; sizeFormatted: string; isVideo: boolean; isAudio: boolean; ext: string }> = [];

    const videoExts = new Set([".mp4", ".mkv", ".avi", ".mov", ".webm", ".flv", ".ts", ".m4v", ".wmv"]);
    const audioExts = new Set([".mp3", ".wav", ".m4a", ".aac", ".flac", ".ogg", ".wma", ".opus"]);
      ...
      try {
        if (entry.isDirectory()) {
          ...
          if (isVideo || isAudio) {
            ...
            const sizeFormatted = size > 1024 * 1024 ? `${sizeMb} MB` : `${Math.round(size / 1024)} KB`;
    ...
    res.json({
      ...
      isDriveMounted,
      currentPath: targetDir,
      parentPath: parentPath !== targetDir ? parentPath : null,
      availableRoots: candidateRoots.filter(r => fs.existsSync(r)),
      folders,
      files
    });
  } catch (err: any) {
    console.error("Colab drive list error:", err);
    res.status(500).json({ success: false, error: err.message || "Failed to list directory contents" });
  }
});

// Colab / Local Drive Direct Import & Transcribe (No OAuth Required)
app.post("/api/colab-drive/import", async (req: any, res: any) => {
  const { filePath, fileName } = req.body;
  const customApiKey = (req.headers["x-gemini-api-key"] as string | undefined)?.trim() || (req.body?.apiKey as string | undefined)?.trim();

  if (!filePath) {
    return res.status(400).json({ error: "مسیر فایل (filePath) الزامی است." });
  }

  const resolvedPath = path.resolve(filePath);
  if (!fs.existsSync(resolvedPath)) {
    return res.status(404).json({ error: `فایل در مسیر یافت نشد: ${resolvedPath}` });
  }

  const actualFileName = fileName || path.basename(resolvedPath);
  const jobId = `trans_job_colab_${Date.now()}`;

  try {
    console.log(`[COLAB DRIVE IMPORT] Starting instant import for: ${resolvedPath}`);
    const fileExt = path.extname(actualFileName).toLowerCase() || ".mp4";
    const safeBaseName = path.basename(actualFileName, fileExt).replace(/[^a-zA-Z0-9_-]/g, "_");
    const uniqueId = `${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
    const localFileName = `colab_${safeBaseName}_${uniqueId}${fileExt}`;
    const destPath = path.join(UPLOADS_DIR, localFileName);
    ...
    transcribeJobs[jobId] = {
      ...
      status: "pending",
      progress: 5,
      message: "در حال آماده‌سازی فایل انتخاب‌شده از کولب/گوگل درایو..."
    };

    // Run import & transcription in background
    (async () => {
      try {
        transcribeJobs[jobId].message = "در حال اتصال فایل از گوگل درایو در کولب...";
        transcribeJobs[jobId].progress = 15;

        // Copy or hardlink file to uploads directory instantly
        try {
          fs.copyFileSync(resolvedPath, destPath);
        } catch (copyErr) {
          // Fallback to stream copy
          await new Promise<void>((resolve, reject) => {
            const rd = fs.createReadStream(resolvedPath);
        ...
        console.log(`[COLAB DRIVE IMPORT] File cloned to uploads: ${destPath}`);
        transcribeJobs[jobId].message = "فایل با موفقیت فراخوانی شد. شروع پردازش صوتی و استخراج دیالوگ‌ها...";
        transcribeJobs[jobId].progress = 25;

        // Delegate to standard transcription background worker
        await runTranscribeBackground(
          jobId,
          { path: destPath, filename: localFileName, originalname: actualFileName },
          undefined,
          customApiKey
        );
      } catch (err: any) {
        console.error("Colab Drive import background failed:", err);
        transcribeJobs[jobId].status = "failed";
        transcribeJobs[jobId].progress = 0;
        transcribeJobs[jobId].error = err.message || "خطا در پردازش فایل انتخاب‌شده.";
        transcribeJobs[jobId].message = `خطا: ${err.message || "فراخوانی فایل ناموفق بود."}`;
      }
    })();

    res.json({ jobId });
  } catch (err: any) {
    console.error("Colab Drive import initialization failed:", err);
    res.status(500).json({ error: err.message || "خطا در اجرای فرآیند فراخوانی فایل." });
  }
});

// Colab Direct Save To Google Drive (No OAuth Required)
app.post("/api/colab-drive/save", async (req: any, res: any) => {
  const { file, targetDir } = req.body;
  if (!file) {
  ...
  if (!fs.existsSync(srcPath)) {
    return res.status(404).json({ error: "فایل خروجی روی سرور یافت نشد." });
  }

  try {
    let destDir = (targetDir as string | undefined)?.trim();
    if (!destDir) {
      if (fs.existsSync("/content/drive/MyDrive")) {
        destDir = "/content/drive/MyDrive/Dubber_Outputs";
      } else if (fs.existsSync("/content/drive")) {
        destDir = "/content/drive/Dubber_Outputs";
      } else {
        destDir = "/content/Dubber_Outputs";
      }
    }
    ...
    console.log(`[COLAB DRIVE SAVE] Saved ${file} to ${destPath}`);
    res.json({
      success: true,
      savedPath: destPath,
      message: `فایل با موفقیت در پوشه ${destPath} ذخیره شد.`
    });
  } catch (err: any) {
    console.error("Colab Drive save failed:", err);
    res.status(500).json({ error: err.message || "خطا در ذخیره‌سازی فایل در گوگل درایو کولب." });
  }
});
